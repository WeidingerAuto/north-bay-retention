...
export default function App() {
  ...
  // Load from storage on mount (window.storage for artifact env, localStorage fallback)
  useEffect(() => {
    (async () => {
      try {
        if (typeof window !== "undefined" && window.storage) {
          const result = await window.storage.get(STORAGE_KEY);
          if (result && result.value) setLiveEntries(JSON.parse(result.value));
        } else {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (typeof window !== "undefined" && window.storage) {
          await window.storage.set(STORAGE_KEY, JSON.stringify(liveEntries));
        } else {
  ...
  function getMonthTotals(year, month) {
    ...
    if (rec.isAggregate) {
      // Aggregate month: add any live entries on top of the stored totals
      const live = liveEntries.filter(e => Number(e.year) === Number(year) && e.month === month);
      const lt = countEntries(live);
      return {
        a: rec.agg.a + lt.a, b: rec.agg.b + lt.b, c: rec.agg.c + lt.c,
        lbb: (rec.agg.lbb||0) + lt.lbb, cb: (rec.agg.cb||0) + lt.cb, ground: (rec.agg.ground||0) + lt.ground,
      };
    }
    return countEntries(rec.entries);
  }
  ...
  function isMonthLocked(year, month) {
    if (unlockedMonths.has(`${year}-${month}`)) return false;
    const now = new Date();
  ...
  function unlockMonth(year, month) {
    setUnlockedMonths(prev => new Set([...prev, `${year}-${month}`]));
  }
  ...
  const currentLiveEntries = liveEntries.filter(e => Number(e.year) === Number(selectedYear) && e.month === selectedMonth);
  // All named entries for this month: historical (from Excel import) + live additions
  const displayEntries = currentRec ? currentRec.entries : [];
      ...
      <main style={{maxWidth:1000,margin:"0 auto",padding:"24px 16px"}}>
        ...
        {view==="entry" && (
          <div>
            ...
            <div className="no-print" style={{background:"white",borderRadius:10,border:`1px solid #38bdf8`,borderTop:`3px solid ${NB_BLUE}`,padding:24,marginBottom:24,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
              ...
              {isMonthLocked(selectedYear, selectedMonth) && (
                <div style={{background:"#fee2e2",border:"1px solid #fca5a5",borderRadius:6,padding:"10px 14px",marginBottom:16,fontSize:13,color:"#991b1b",fontWeight:600,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
                  <span>🔒 This month is locked — {MONTH_FULL[selectedMonth]} {selectedYear} was locked 60 days after the end of the month. To unlock this month press the Unlock button.</span>
            ...
            {/* Current month entries — below tallies */}
            {displayEntries.length > 0 && (
              <div className="no-print" style={{background:"white",borderRadius:10,border:"1px solid #38bdf8",overflow:"hidden",marginTop:20}}>
                <div style={{padding:"14px 20px",borderBottom:"1px solid #f1f5f9",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <span style={{fontSize:13,color:"#64748b"}}>{displayEntries.length} entries this month</span>
                </div>
                  ...
                  <tbody>
                    {displayEntries.map((e,i)=>(
                      <tr key={e.id} style={{borderTop:"1px solid #f1f5f9",background:i%2===0?"white":"#fafafa"}}>
                        ...
                        <td style={{padding:"10px 16px"}}>
                          {!isMonthLocked(selectedYear, selectedMonth) && !e.id?.startsWith("hist-") && <button onClick={()=>deleteEntry(e.id)} style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:18,lineHeight:1}}>×</button>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
