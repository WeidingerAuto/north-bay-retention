...
export default function App() {
  ...
  const [editForm, setEditForm] = useState({name:"",outcome:"A",disposition:"CB",broker:"",ourCustomer:true});
    ...
    try {
      const res = await apiFetch(`/api/entries/${editingId}`, {
        ...
        body:JSON.stringify({
          ...
          our_customer:editForm.ourCustomer,
        }),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
                      ...
                      if (editingId === e.id) {
                        return (
                          <tr key={e.id} style={{borderTop:"1px solid #38bdf8",background:"#f0f9ff"}}>
                            <td style={{padding:"6px 10px",fontSize:12,color:"#94a3b8"}}>{formatDate(e.timestamp||e.created_at)}</td>
