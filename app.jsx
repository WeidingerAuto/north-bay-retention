...
export default function App() {
  ...
  const [editForm, setEditForm] = useState({name:"",outcome:"A",disposition:"CB",broker:""});
  ...
  function isMonthLocked(year, month) {
    ...
    // Last day of the record's month
    const monthEnd = new Date(year, monthIndex + 1, 0);
    // Add 60 days to month-end
    const lockDate = new Date(monthEnd.getTime() + 60 * 24 * 60 * 60 * 1000);
    return now > lockDate;
  }
  ...
  function startEdit(entry) {
    ...
    setEditForm({name:entry.name,outcome:entry.outcome,disposition:entry.disposition||"CB",broker:entry.broker||""});
  }

  function cancelEdit() {
    setEditingId(null);
    setEditForm({name:"",outcome:"A",disposition:"CB",broker:""});
  }

  function saveEdit(id) {
    setLiveEntries(prev => prev.map(e => e.id===id ? {...e, name:editForm.name.trim()||e.name, outcome:editForm.outcome, disposition:editForm.disposition, broker:editForm.broker.trim()} : e));
    setEditingId(null);
    setEditForm({name:"",outcome:"A",disposition:"CB",broker:""});
  }
                ...
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  ...
                  <tbody>
                    {currentEntries.filter(e=>!e.id?.startsWith("hist-")).map((e,i)=>{
                      ...
                      return (
                      <tr key={e.id} style={{borderTop:"1px solid #f1f5f9",background:isEditing?"#eff6ff":i%2===0?"white":"#fafafa"}}>
                        <td style={{padding:"10px 16px",fontWeight:500}}>
                          {isEditing ? (
                            <input value={editForm.name} onChange={ev=>setEditForm(f=>({...f,name:ev.target.value}))}
                              style={{width:"100%",padding:"4px 8px",border:"1px solid #93c5fd",borderRadius:4,fontSize:13,boxSizing:"border-box"}}/>
                          ) : e.name}
                        </td>
                        <td style={{padding:"10px 16px"}}>
                          {isEditing ? (
                            <select value={editForm.outcome} onChange={ev=>setEditForm(f=>({...f,outcome:ev.target.value}))}
                              style={{padding:"4px 8px",border:"1px solid #93c5fd",borderRadius:4,fontSize:12}}>
                          ...
                          {!locked && (
                            isEditing ? (
                              <span style={{display:"flex",gap:6}}>
                                <button onClick={()=>saveEdit(e.id)} style={{background:"#16a34a",color:"white",border:"none",borderRadius:4,padding:"3px 10px",fontSize:12,fontWeight:600,cursor:"pointer"}}>Save</button>
                                <button onClick={cancelEdit} style={{background:"#94a3b8",color:"white",border:"none",borderRadius:4,padding:"3px 10px",fontSize:12,fontWeight:600,cursor:"pointer"}}>Cancel</button>
                              </span>
                            ) : (
                              <span style={{display:"flex",gap:6}}>
                                <button onClick={()=>startEdit(e)} style={{background:"none",border:"none",color:NB_BLUE,cursor:"pointer",fontSize:13,fontWeight:600}}>✎</button>
